...
//						+ ViewHelper.getAlpha(v));
    ...
    private void animateZoom(View left, View right, View other, float positionOffset,
                             boolean in) {
        if (mState != State.IDLE) {
            if (left != null) {
                manageLayer(left, true);
                ...
                ViewHelper.setPivotY(left, left.getMeasuredHeight() * 0.5f);
                ViewHelper.setScaleY(left, mScale);//缩小高度
            }
            if (right != null) {
                manageLayer(right, true);
            ...
            if (other != null) {
                manageLayer(other, true);
                mScale = in ? ZOOM_MAX + (1 - ZOOM_MAX) * (1 - positionOffset)
                        : 1 + ZOOM_MAX - ZOOM_MAX * (1 - positionOffset);
                ViewHelper.setPivotX(other, right.getMeasuredWidth() * 0.5f);
                ViewHelper.setPivotY(other, right.getMeasuredHeight() * 0.5f);
                ViewHelper.setScaleY(other, mScale);
            }
        }
    }
    ...
    public void onPageScrolled(int position, float positionOffset,
        ...
        mOther = findViewFromObject(position + 2);
        ...
        switch (mEffect) {
            ...
            case Stack:
                ...
                break;
            case ZoomIn:
                animateZoom(mLeft, mRight, mOther, effectOffset, true);
                break;
            case ZoomOut:
                animateZoom(mLeft, mRight, mOther, effectOffset, false);
                break;
            case RotateUp:
